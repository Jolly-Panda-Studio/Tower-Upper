...
public class JSONObject {
	...
	public void AddField(string name, System.Enum val)
	{
	...
	public void SetField(string name, string val) { SetField(name, Create(val)); }
...
{
	/// <summary>
	/// convert array fild of json to list of string
	/// </summary>
	/// <param name="json"></param>
	/// <param name="tag"></param>
	/// <returns></returns>
	public static List<string> GetStringsList(this JSONObject json, string tag)
	{
		List<string> output = new List<string>();
		for (int i = 0; i < json[tag].Count; i++)
		{
			output.Add(json[tag][i].str);
		}
		return output;
	}

	public static List<int> GetIntegersList(this JSONObject json, string tag)
	{
		List<int> output = new List<int>();
		for (int i = 0; i < json[tag].Count; i++)
		{
			output.Add((int)json[tag][i].n);
		}
		return output;
	}

	public static JSONObject UpdateJson<T>(this JSONObject json, string tag, T value)
	{
		if (value is int intNum)
		{
			json[tag].n = intNum;
		}
		else if (value is float floatNum)
		{
			json[tag].n = floatNum;
		}
		...
		{
			UnityEngine.Debug.LogError("Nothing for change!!");
		}
